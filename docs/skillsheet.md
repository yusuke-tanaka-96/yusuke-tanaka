...
AWSを中心としたクラウドインフラの設計・構築を専門とするインフラエンジニアです。Terraformによる大規模なIaC開発と、最大6名のインフラチームリーダーとしてAWS基盤構築プロジェクトを本番リリースまで牽引した経験を強みとしています。
...
- **生成AIを使った開発の実践**：Claude Codeを実装・テスト・設計是正の主要な手段として常用。着手前に「守るべき挙動と受入条件」を定めてから実装に入る型（AI-DLC）で70案件を推進し、AIが生成した成果物の品質にはドメインオーナーとして責任を持つ
- **課題の発見から是正までの内製化**：総合試験と実装の突合で不具合を自ら発見し、Issue起票・チーム共有・修正までを一人で完結。調査と裏取りにはAIエージェントを並列に走らせ、他メンバーがそのまま着手できる証跡付きのIssueとして残す
- **対応範囲の広さ**：AWS / Terraform（インフラ）、Java / Spring Boot（バックエンド）、React / TypeScript（フロントエンド）、テスト設計までを一人で通せる
...
| 技術 | 経験年数 | レベル | 備考 |
| ---- | ---- | ---- | ---- |
| Java（Java 25） | 約3ヶ月 | ○ | Spring Modulithによるモジュラーモノリスで会計・レジ機能を実装。本体407ファイル・REST API 31本を新規実装 |
| Spring Boot / Spring Modulith | 約3ヶ月 | ○ | ドメインイベント連携、ユースケース実装、JPAによる永続化 |
| TypeScript / React 19 | 約5ヶ月 | ○ | POS（レジ・精算・会計設定・プリンター連携）1,054ファイルを実装。Vite / TanStack Query / Zustand |
...
| AI-DLC（AI駆動開発ライフサイクル） | 約3ヶ月 | ○ | コンテキスト整理→合意形成→テスト契約→実装計画→実装差分の5フェーズを70案件で実践 |
...
| 総合テスト | ○ | 試験設計・台帳整備を会計/レジ/税ドメインで担当（台帳13,823行中2,890行を執筆）、担当シナリオ20本を作成・実施 |
...
**主な成果**
- 参画から約5.5ヶ月でPR 176本をマージ。フロントエンド実装から着手し、バックエンド・DB・インフラ・試験設計へ担当領域を拡大
- **AI駆動開発の実践**：Claude Codeを主要な実装手段として使用し、全コミットの8割をAIとの共著として記録。着手前にテスト契約（守るべき挙動と受入条件）を定めてから実装に入る型（AI-DLC）を70案件で回した
- **バグの発見から是正までの内製化**：総合試験と実装の突合で検出した不具合をGitHub Issue 159件として起票し、うち104件を自身のPRで解決。調査・裏取りにはAIエージェントを並列に走らせ、Issue本文にコード箇所と設計書IDを引用して、他メンバーがそのまま着手できる形で共有した
